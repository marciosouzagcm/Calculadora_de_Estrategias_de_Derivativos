export class VolatilityBase {
    ...
    calculateMetrics(allOptions, spotPrice, feePerLeg) {
        ...
        allOptions.forEach(opt => {
            if (!opt.vencimento || opt.tipo === 'SUBJACENTE')
                return;
            const dateStr = String(opt.vencimento).split(/[T ]/)[0];
            if (!expirationGroups[dateStr])
                expirationGroups[dateStr] = [];
            ...
            for (const call of calls) {
                for (const put of puts) {
                    ...
                    if (this.isStraddle) {
                        ...
                        if (Math.abs(avgStrike - spotPrice) / spotPrice > 0.20)
                            continue;
                    }
                    const premiumTotal = call.premio + put.premio;
                    if (premiumTotal < 0.05)
                        continue;
                    // --- Cálculo de Gregas ---
                    ...
                    results.push({
                        ...
                        natureza: (this.isLong ? 'DÉBITO' : 'CRÉDITO'),
    ...
    getDescription() {
        if (this.isStraddle)
            return this.isLong ? 'Compra de Call e Put no mesmo strike.' : 'Venda de Call e Put no mesmo strike.';
        return this.isLong ? 'Compra de Call e Put com strikes diferentes.' : 'Venda de Call e Put com strikes diferentes.';
    }
}
// Implementações concretas
export class LongStraddle extends VolatilityBase {
    constructor() {
        super(...arguments);
        this.name = 'Long Straddle';
        this.isLong = true;
        this.isStraddle = true;
        this.marketView = 'VOLÁTIL';
    }
}
export class ShortStraddle extends VolatilityBase {
    constructor() {
        super(...arguments);
...
export class LongStrangle extends VolatilityBase {
    constructor() {
        super(...arguments);
...
export class ShortStrangle extends VolatilityBase {
    constructor() {
        super(...arguments);
