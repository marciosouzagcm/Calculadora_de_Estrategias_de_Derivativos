...
export class LongStraddle {
    ...
    calculateMetrics(allOptions, assetPrice, feePerLeg) {
        ...
        allOptions.forEach(leg => {
            if (!leg.vencimento || leg.tipo === 'SUBJACENTE' || leg.premio <= 0)
                return;
        ...
        for (const date in groupsByDate) {
            ...
            for (const call of calls) {
                for (const put of puts) {
                    ...
                    if (strikeDiff > (avgStrike * 0.006))
                        continue;
                    // Filtro de Proximidade: Straddles são montados "At-The-Money" (ATM)
                    if (Math.abs(avgStrike - assetPrice) / assetPrice > 0.10)
                        continue;
