...
export class ShortStraddle {
    constructor() {
        this.name = 'Short Straddle (Venda)';
    ...
    calculateMetrics(allOptions, assetPrice, feePerLeg) {
        ...
        allOptions.forEach(leg => {
            if (!leg.strike || leg.tipo === 'SUBJACENTE' || leg.premio <= 0)
                return;
            if (!optionsByStrike[leg.strike])
                optionsByStrike[leg.strike] = [];
