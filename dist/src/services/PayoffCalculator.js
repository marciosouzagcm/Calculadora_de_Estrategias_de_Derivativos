...
export class PayoffCalculator {
    constructor(optionsData, feePerLeg, lotSize) {
        this.optionsData = optionsData;
        this.feePerLeg = feePerLeg;
        this.lotSize = lotSize;
    }
    normalizeMetrics(metrics, currentAssetPrice) {
        ...
        metrics.riskRewardRatio = metrics.max_profit > 0
            ? parseFloat((metrics.max_loss / metrics.max_profit).toFixed(2))
            : 99;
        metrics.pernas = metrics.pernas.map((p) => {
            if (p.derivative.strike > 500)
                p.derivative.strike /= 100;
            if (p.derivative.premio > 50)
                p.derivative.premio /= 100;
            return p;
        });
        return metrics;
    }
    calculateNetGreeks(metrics, currentAssetPrice) {
        const SELIC = 0.1075;
        return metrics.pernas.reduce((acc, leg) => {
            ...
            if (leg.derivative.gregas_unitarias && leg.derivative.gregas_unitarias.delta !== 0) {
                ...
                const tipo = leg.derivative.tipo;
                unitarias = {
    ...
    findTwoLegCombinationsDifferentType(options, mustHaveSameStrike = false) {
        const combinations = [];
        ...
        for (const callLeg of calls) {
            for (const putLeg of puts) {
                if (callLeg.vencimento !== putLeg.vencimento)
                    continue;
    ...
    findAndCalculateSpreads(currentAssetPrice, maxRR = 3.0) {
        ...
        for (const sObj of strategiesToRun) {
            let combos = [];
            if (sObj.strategy instanceof BullCallSpread || sObj.strategy instanceof BearCallSpread)
                combos = sameCall;
                ...
                combos = sameStrike;
            else if (sObj.strategy instanceof LongStrangle || sObj.strategy instanceof ShortStrangle)
                combos = diffStrike;
                ...
                try {
                    ...
                    if (res) {
                        ...
                        for (const item of resArray) {
                            ...
                            if (normalized.riskRewardRatio <= maxRR)
        ...
        return results.sort((a, b) => a.riskRewardRatio - b.riskRewardRatio);
    }
}
