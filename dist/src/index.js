...
function prepareStrategyForFrontend(metrics, lot) {
...
app.get('/api/analise', async (req, res) => {
    try {
        const ticker = (req.query.ticker || 'ABEV3').toUpperCase();
        const inputLot = parseInt(req.query.lote || '1000');
        const currentPrice = await DatabaseService.getSpotPrice(ticker);
        if (currentPrice === 0)
            throw new Error(`Preço spot não encontrado para ${ticker}`);
