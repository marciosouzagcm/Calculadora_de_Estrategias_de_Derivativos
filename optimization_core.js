...
/**
 * Array de opções simulando o livro de ofertas. 
 * AGORA INCLUI PUTS COM STRIKES COMPATÍVEIS PARA UMA PUT SPREAD.
 */
const MOCK_OPTIONS_DATA = [
    ...
    { symbol: "PETRH370", type: 'CALL', strike: 37.00, premium: 0.40, contracts: 1, position: 'BUY' },  // Buy Call
    
    // Opções Put (Ex: PETRW) - AJUSTADO PARA PERMITIR PUT SPREAD (35.00 > 34.00)
    ...
    { symbol: "PETRW330", type: 'PUT', strike: 33.00, premium: 0.80, contracts: 1, position: 'SELL' }, // Put extra para variação
];
...
/**
 * Calcula o payoff de uma única opção em um dado preço futuro do ativo.
 * O Payoff Líquido inclui o custo/crédito do prêmio inicial, mas não as taxas.
 * * @param {Option} option 
 * @param {number} assetPrice - Preço do ativo na expiração
 * @returns {number} Payoff líquido TOTAL (R$)
 */
function calculatePayoff(option, assetPrice) {
    ...
    if (option.position === 'BUY') {
        // Lucro/Prejuízo = Valor Intrínseco - Prêmio Pago
        payoffPerContract = intrinsicValue - option.premium;
    } else { // SELL
        // Lucro/Prejuízo = Prêmio Recebido - Valor Intrínseco
...
function calculateStrategyMetrics(strategyLegs) {
    let initialCashFlow = 0; // Fluxo de caixa inicial (Custo/Crédito)
    ...
    const minStrike = Math.min(...strategyLegs.map(l => l.strike)) - 5;
    const maxStrike = Math.max(...strategyLegs.map(l => l.strike)) + 5;
    ...
    priceRange.push(DEFAULT_ASSET_PRICE, ...strategyLegs.map(l => l.strike));
    ...
    // O Payoff já está líquido de prêmio, mas precisamos adicionar as taxas do initialCashFlow
    const testPrices = [...uniquePrices, 0.01, 1000.00]; // Preço próximo de zero e preço muito alto

    testPrices.forEach(price => {
        let totalPayoffAtPrice = 0;
        strategyLegs.forEach(leg => {
            // O calculatePayoff já retorna o valor líquido com o prêmio (custo/crédito).
            totalPayoffAtPrice += calculatePayoff(leg, price);
        });
        
        // Ajustamos o Payoff com o impacto das taxas de corretagem
        // O Payoff da função 'calculatePayoff' é (Valor Intrínseco +/- Prêmio).
        // A diferença entre a soma dos prêmios (sem taxa) e o initialCashFlow é o total de taxas.
        const totalFees = -initialCashFlow - strategyLegs.reduce((sum, leg) => {
            const flow = leg.premium * leg.contracts * LOT_SIZE;
            return sum + (leg.position === 'BUY' ? -flow : flow);
        }, 0);

        // Payoff Líquido Final = Payoff Bruto do Prêmios + Payoff na Expiração - Taxas
        // Como 'calculatePayoff' já considera (Prêmio +/- Valor Intrínseco),
        // basta subtrair as taxas totais do resultado final.
        totalPayoffAtPrice -= totalFees; 
        
        allPayoffs.push(totalPayoffAtPrice);
    });
    ...
    for (let i = 1; i < uniquePrices.length; i++) {
        ...
        // Calcula o Payoff líquido (incluindo prêmio) e as taxas
        const totalFees = -initialCashFlow - strategyLegs.reduce((sum, leg) => {
            const flow = leg.premium * leg.contracts * LOT_SIZE;
            return sum + (leg.position === 'BUY' ? -flow : flow);
        }, 0);
        ...
        strategyLegs.forEach(leg => {
            payoff1 += calculatePayoff(leg, p1);
            payoff2 += calculatePayoff(leg, p2);
        });
        ...
        if ((payoff1 <= 0 && payoff2 > 0) || (payoff1 >= 0 && payoff2 < 0)) {
            ...
            // Adiciona se não for muito próximo de um strike (que já foi testado)
            if (!breakEvens.some(be => Math.abs(be - pe) < 0.1)) {
                breakEvens.push(pe);
            }
        }
    }
    ...
    return {
        ...
        initialCost: round2(initialCashFlow),
        legs: strategyLegs,
    };
}
...
// =========================================================================
// 5. LÓGICA DE OTIMIZAÇÃO (O "CORE" DO PROJETO)
// =========================================================================
...
// =========================================================================
// 6. EXECUÇÃO DO "COMANDO" PRINCIPAL
// =========================================================================
