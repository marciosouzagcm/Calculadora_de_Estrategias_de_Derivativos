        ...
        for (let i = 0; i < calls.length - 2; i++) {
            for (let j = i + 1; j < calls.length - 1; j++) {
                for (let k = j + 1; k < calls.length; k++) {
                    ...
                    if (k1Low.vencimento !== k2Mid.vencimento || k2Mid.vencimento !== k3High.vencimento) continue;
                    ...
                    if (Math.abs(leftWing - rightWing) > 0.05) continue; 
                     ...
                     */
                    if (cost <= 0.01 || cost >= leftWing) continue;
